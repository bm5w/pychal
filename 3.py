"""Solution to python challenge # 3:
...
def three(inp):
    ...
    for num, x in enumerate(inp):
        if num >= 4 and num < len(inp)-4:
            if all([x.islower(),
                    inp[num-3:num].isupper(),
                    inp[num+1:num+4].isupper(),
                    inp[num-4].islower(),
                    inp[num+4].islower()]):
                output += x
    return output
...
if __name__ == "__main__":
    inp = u"""kAewtloYgcFQaJNhHVGxXDiQmzjfcpYbzxlWrVcqsmUbCunkfxZWDZjUZMiGqhRRiUvGmYmvnJIHEmbT
